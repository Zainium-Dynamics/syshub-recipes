...
# g-ir-scanner (gobject-introspection) dlopens the library it's
# scanning mid-build -- this cascades into libcurl's OWN runtime deps
# too, all confirmed via real appstream -Dgir=true CI failures.
# wayland-client comes from the real Zainium wayland package instead
# (needs_toolchain), not borrowed here.
for lib in libz.so.1 liblzma.so.5 libexpat.so.1 libxml2.so.2 \
    ...
    libpcre2-8.so.0 libffi.so.8 libgirepository-1.0.so.1 libmount.so.1 \
    libcares.so.2 libnghttp2.so.14 libidn2.so.0 libunistring.so.5 \
    libpsl.so.5 libssl.so.3 libcrypto.so.3 libbrotlidec.so.1 \
